 ...
 *
 * Env:
 *   ORG        - required, your org login (e.g., "my-org")
 *   GH_TOKEN   - recommended: a PAT with 'read:org' and 'repo' if you want private repo stats
 *                fallback: GITHUB_TOKEN from Actions (public repos typically OK)
 *
 * Notes:
 * - By default excludes forks and archived repos. Tweak flags below if you want them.
 */
const fs = require("fs");
...
if (!ORG) {
  console.error("Missing ORG environment variable.");
  process.exit(1);
}
if (!GH_TOKEN) {
...
// privacy: null => both; "PUBLIC" or "PRIVATE" to restrict
const PRIVACY = null; // or "PUBLIC" | "PRIVATE"
...
// One page of repos at a time, aggregating counts per repo.
...
async function fetchAllRepos() {
  ...
  while (true) {
    ...
    if (!INCLUDE_ARCHIVED) {
      nodes = nodes.filter((r) => !r.isArchived);
    }
    all.push(...nodes);
...
function renderMarkdown(stats, topRepos) {
  ...
  if (topRepos.length) {
    ...
    for (const r of topRepos) {
      lines.push(
        `| [${r.name}](https://github.com/${ORG}/${r.name}) | ${formatNumber(
          r.commits
        )} | ${formatNumber(r.issuesOpen)} | ${formatNumber(
          r.prsOpen
